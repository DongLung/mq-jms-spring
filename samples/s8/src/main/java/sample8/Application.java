...
public class Application {
  ...
  public static void main(String[] args) {
    ...
    // Keep going until the listener tells us to stop
    while (ok) {
      try {
    ...
    // Messages may continue to be delivered (or attempted to be delivered) until after exit() completes.
    // Which may take some time. This might take a little while, and generate
    // exception stacks as the listener container ends, but at least it does finish.
  ...
  // We want to apply an error handler to the listener, which can be done by adding it to the factory.
  // The listener also needs to be transactional, to allow the repeated rollbacks to occur.
  @Bean
